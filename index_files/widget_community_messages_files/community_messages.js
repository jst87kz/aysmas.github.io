var CommunityWidget = {
  ...
  lpGetKey: function () {
    var stNow = vkNow();
    ajax.post('widget_community_messages.php', {act: 'get_key'}, {
  ...
  pushEvent: function(ev_text) {
    var ev = ev_text.split('<!>'), ev_ver = ev[0], ev_type = ev[1], peer = ev[2];
...
<div class="wcm_msg">\
  ...
  '+fwd+'\
    ...
    if (before) {
      var block = ge('block_'+this.before_block), first = block.firstChild;
      if (first) {
  ...
  prepareText: function(text, opts) {
    ...
    text = text.replace(/(^|[^A-Za-z0-9À-ßà-ÿ¸¨\-\_])(https?:\/\/)?((?:[A-Za-z\$0-9À-ßà-ÿ¸¨](?:[A-Za-z\$0-9\-\_À-ßà-ÿ¸¨]*[A-Za-z\$0-9À-ßà-ÿ¸¨])?\.){1,5}[A-Za-z\$ðôóêîíëàéíñòÐÔÓÊÎÍËÀÉÍÑÒ\-\d]{2,22}(?::\d{2,5})?)((?:\/(?:(?:\&amp;|\&#33;|,[_%]|[A-Za-z0-9À-ßà-ÿ¸¨\-\_#%?+\/\$.~=;:]+|\[[A-Za-z0-9À-ßà-ÿ¸¨\-\_#%?+\/\$.,~=;:]*\]|\([A-Za-z0-9À-ßà-ÿ¸¨\-\_#%?+\/\$.,~=;:]*\))*(?:,[_%]|[A-Za-z0-9À-ßà-ÿ¸¨\-\_#%?+\/\$.~=;:]*[A-Za-z0-9À-ßà-ÿ¸¨\_#%?+\/\$~=]|\[[A-Za-z0-9À-ßà-ÿ¸¨\-\_#%?+\/\$.,~=;:]*\]|\([A-Za-z0-9À-ßà-ÿ¸¨\-\_#%?+\/\$.,~=;:]*\)))?)?)/ig, function () {
  ...
  send: function(opts) {
    ...
    if (opts.retry) {
      ...
      if (opts.media) {
        ...
        var txt = ge('send_text'), text = String(Emoji.editableVal(txt)).trim();
        if (!text) {
          return;
        }
        val(txt, '');
  ...
  showErrorTools: function(el, msg_id) {
    showTooltip(el, {
      ...
      onShowStart: function(tt) {
        var left = getXY(tt.container)[0], width = getSize(tt.container)[0];
  ...
  onNewMessage: function(ev) {
    ...
    var data = {
      id: msg_id,
      text: this.prepareText(text, {no_clean: 1}),
  ...
  onChooseFiles: function(type, files) {
    for(var i = 0; i < files.length; i++) {
      var file = files[i], try_check_file = 0;
      if (type == 'photo') {
  ...
  onUploadDone: function(file, params) {
    ...
    switch(file._type) {
      case 'photo':
        ...
        info.xhr = ajax.post('al_photos.php', extend({act: 'choose_uploaded'}, params), {
...
try{ stManager.done('api/widgets/community_messages.js'); }catch(e) {}
